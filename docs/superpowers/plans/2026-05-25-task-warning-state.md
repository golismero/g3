...
Per-plugin authoring; audit all wrappers and produce a "native condition → {DONE, WARNING, ERROR}" table per wrapper, which becomes the Tier 4 plan. Highlights: piped shell wrappers (`nmap`/`subfinder`/`wafw00f`) must capture the tool's real exit status instead of `tee`'s; non-piped (`nikto`/`dig`) suppress benign non-zeros to 0; Python (`testssl`/`hydra`) stop ignoring `returncode` and fold per-target outcomes into one exit code. The testssl CAA-DNS case is adjudicated here (benign → 0; `scanProblem`/no data → non-zero+empty → ERROR).
