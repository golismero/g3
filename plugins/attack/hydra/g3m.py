...
def main():
    ...
    for issue in input_array:
        ...
        for propname in ("_cmd", "_fp", "_start", "_end", "severity", "affects", "taxonomy", "references", "credentials"):
    ...
    for issue in input_array:
        ...
        for x in issue["credentials"]:
            credentials.append((x["host"], x["port"], x["service"], x["login"], x["password"]))
    ...
    credentials = sorted(set(credentials))
    for x in credentials:
        merged_issue["credentials"].append({
            "host": x[0],
            "port": x[1],
            "service": x[2],
            "login": x[3],
