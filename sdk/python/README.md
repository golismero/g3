...
| 2 | `g3client.scanner` | Event-handler base class for **orchestrated** scans (the `g3cli` flow): subclass `Scanner`, override `on_progress`/`on_log`, and `scan()` launches, waits, and returns a report. |
