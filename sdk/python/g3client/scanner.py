...
`Scanner` reproduces, in Python, the end-to-end g3cli flow: upload imports, build the
script DSL, launch the scan, wait for completion, then dispatch a reporter task and
download its artifact.

It is an event-handler base class: subclass it and override `on_progress` / `on_log`
to observe a scan as it runs (the push path). Everything else about the scan — per-
task state, per-task artifacts, task-level logs — is read on demand straight from
`g3client.api` (Tier 1) keyed by the scan id (the pull path); `Scanner` deliberately
does not re-wrap that, since it would be a trivial, redundant shim over Tier 1.
...
class Scanner:
    """Event-handler base class for orchestrated scans, built only on `ApiClient`.

    Subclass and override `on_progress` / `on_log` to observe a running scan; the
    base implementations are no-ops. The log endpoint is polled only when `on_log`
    is overridden, so the default subclass-free `Scanner` does no extra log fetches.
    """
    ...
    def on_progress(self, progress: ScanProgress) -> None:
        """Called with each scan-progress snapshot as it is polled.

        Override to observe progress; the base implementation does nothing.
        """

    def on_log(self, entries: list[dict[str, Any]]) -> None:
        """Called with each batch of new scan-level log entries as they appear.

        Override to observe logs; the base implementation does nothing. The log
        endpoint is polled only when this method is overridden.
        """
    ...
    def scan(
        ...
        *,
        ...
        """
        api = self.api
        # Poll the log endpoint only if a subclass actually consumes logs; the
        # default no-op `on_log` should cost zero extra requests.
        wants_logs = type(self).on_log is not Scanner.on_log
        ...
        def on_poll(progress: Optional[ScanProgress]) -> None:
            ...
            if wants_logs:
                _emit_new_logs(api, scanid, log_mark, self.on_log)
